...
class AppStore extends Component {
  ...
  onClickChange = tabValue => {
    this.setState({currentTabId: tabValue})
  }

  filteredResults = () => {
    const {currentTabId, initialAppList} = this.state
    const filteredList = initialAppList.filter(
  ...
  render() {
    const {currentTabId, searchInput} = this.state
    const filteredList = this.filteredResults()
    const searchResults = filteredList.filter(eachSearch =>
      ...
      <div className="bg-container">
        ...
        <ul className="app-container">
          {searchResults.map(eachApp => (
            <AppItem eachAppList={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
